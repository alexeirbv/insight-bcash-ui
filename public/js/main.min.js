...
var defaultLanguage = localStorage.getItem("insight-language") || "en",
    defaultCurrency = localStorage.getItem("insight-currency") || "BCH";
angular.module("insight", ["ngAnimate", "ngResource", "ngRoute", "ngProgress", "ui.bootstrap", "ui.route", "monospaced.qrcode", "gettext", "angularMoment", "insight.system", "insight.socket", "insight.blocks", "insight.transactions", "insight.address", "insight.search", "insight.status", "insight.connection", "insight.currency", "insight.addressFormat", "insight.messages"]), angular.module("insight.system", []), angular.module("insight.socket", []), angular.module("insight.blocks", []), angular.module("insight.transactions", []), angular.module("insight.address", []), angular.module("insight.search", []), angular.module("insight.status", []), angular.module("insight.connection", []), angular.module("insight.currency", []), angular.module("insight.addressFormat", []), angular.module("insight.messages", []), angular.module("insight.address").controller("AddressController", function($scope, $rootScope, $routeParams, $location, Global, Address, getSocket) {
    $scope.global = Global;
    ...
    socket.on("connect", function() {
        ...
        $rootScope.currentAddr = $routeParams.addrStr, _startSocket(), Address.get({
            ...
            address.addrStr = $rootScope.formatAddress(address.addrStr), $rootScope.titleDetail = address.addrStr.substring(0, 7) + "...", $rootScope.flashMessage = null, $scope.address = address
        }, function(e) {
...
}), angular.module("insight.addressFormat").controller("AddressFormatController", function($scope, $rootScope, $route) {
    var formats = [{
        name: "CashAddr",
    ...
    $scope.formats = formats, $rootScope.formatAddress = function(address) {
        try {
        ...
        return address
    }, $rootScope.addressFormat = formats[0], localStorage.getItem("insight-addressFormat") && ($rootScope.addressFormat = formats.find(function(x) {
        return x.name == localStorage.getItem("insight-addressFormat")
    })), $scope.setAddressFormat = function(addressFormat) {
        $rootScope.addressFormat = addressFormat, localStorage.setItem("insight-addressFormat", addressFormat.name), $route.reload()
    }
    ...
    $rootScope.currency.getConvertion = function(value) {
        if (value = 1 * value, !isNaN(value) && "undefined" != typeof value && null !== value) {
            ...
            var response;
            return "USD" === this.symbol ? response = _roundFloat(value * this.factor, 2) : "mBCH" === this.symbol ? (this.factor = 1e3, response = _roundFloat(value * this.factor, 5)) : "bits" === this.symbol ? (this.factor = 1e6, response = _roundFloat(value * this.factor, 2)) : (this.factor = 1, response = value), 1e-7 > response && (response = response.toFixed(8)), response + " " + this.symbol
        }
        return "value error"
        ...
        }) : "mBCH" === currency ? $rootScope.currency.factor = 1e3 : "bits" === currency ? $rootScope.currency.factor = 1e6 : $rootScope.currency.factor = 1
...
angular.module("insight.system").controller("IndexController", function($scope, Global, getSocket, Blocks) {
    ...
    qrcode.callback = function(data) {
        _scanStop();
        var str = 0 === data.indexOf("bitcoincash:") ? data.substring(8) : data;
    ...
    var pageNum = 0,
        ...
        _processTX = function(tx) {
            tx.vin = tx.vin.map(function(vin) {
                return vin.addr && (vin.addr = $rootScope.formatAddress(vin.addr)), vin
            }), tx.vout = tx.vout.map(function(vout) {
                return vout.scriptPubKey.addresses && vout.scriptPubKey.addresses.map && (vout.scriptPubKey.addresses = vout.scriptPubKey.addresses.map(function(addr) {
...
angular.module("insight").directive("scroll", function($window) {
    ...
    gettextCatalog.setStrings("de_DE", {
        ...
        '<strong>insight</strong> is still in development, so be sure to report any bugs and provide feedback for improvement at our <a href="https://github.com/bitpay/insight/issues" target="_blank">github issue tracker</a>.': '<strong>insight</strong> befindet sich aktuell noch in der Entwicklung. Bitte sende alle gefundenen Fehler (Bugs) und Feedback zur weiteren Verbesserung an unseren <a href="https://github.com/bitpay/insight/issues" target="_blank">Github Issue Tracker</a>.',
        About: "Über insight",
        Address: "Adresse",
        Age: "Alter",
        "Application Status": "Programmstatus",
        "Best Block": "Bester Block",
        "Bitcoin node information": "Bitcoin-Node Info",
        Block: "Block",
        "Block Reward": "Belohnung",
        Blocks: "Blöcke",
        "Bytes Serialized": "Serialisierte Bytes",
        "Can't connect to bitcoind to get live updates from the p2p network. (Tried connecting to bitcoind at {{host}}:{{port}} and failed.)": "Es ist nicht möglich mit Bitcoind zu verbinden um live Aktualisierungen vom P2P Netzwerk zu erhalten. (Verbindungsversuch zu bitcoind an {{host}}:{{port}} ist fehlgeschlagen.)",
        ...
        "Last Block Hash (Bitcoind)": "Letzter Hash (Bitcoind)",
        ...
        "Best Block": "最良ブロック",
        "Bitcoin comes with a way of signing arbitrary messages.": "Bitcoinには任意のメッセージを署名する昨日が備わっています。",
        "Bitcoin node information": "Bitcoinノード情報",
        Block: "ブロック",
        "Block Reward": "ブロック報酬",
        Blocks: "ブロック",
        "Broadcast Raw Transaction": "生のトランザクションを配信",
        "Bytes Serialized": "シリアライズ後の容量 (バイト)",
        "Can't connect to bitcoind to get live updates from the p2p network. (Tried connecting to bitcoind at {{host}}:{{port}} and failed.)": "P2Pネットワークからライブ情報を取得するためにbitcoindへ接続することができませんでした。({{host}}:{{port}} への接続を試みましたが、失敗しました。)",
        ...
        Input: "入力",
        "Last Block": "直前のブロック",
        "Last Block Hash (Bitcoind)": "直前のブロックのハッシュ値 (Bitcoind)",
        ...
        "This form can be used to broadcast a raw transaction in hex format over\n        the Bitcoin network.": "このフォームでは、16進数フォーマットの生のトランザクションをBitcoinネットワーク上に配信することができます。",
        "This form can be used to verify that a message comes from\n        a specific Bitcoin address.": "このフォームでは、メッセージが特定のBitcoinアドレスから来たかどうかを検証することができます。",
        "Time Offset": "時間オフセット",
        Timestamp: "タイムスタンプ",
        Today: "今日",
        "Total Amount": "Bitcoin総量",
        "Total Received": "総入金額",
        "Total Sent": "総送金額",
